...
(function () {
    ...
    window.h3dBgSetFlyingFretLabelVisible = (v) => _bgWriteGlobal('flyingFretLabelVisible', !!v);
    window.h3dBgSetChordBaseFretLabelsVisible = (v) => _bgWriteGlobal('chordBaseFretLabelsVisible', !!v);
    ...
    function createFactory() {
        ...
        // Flying per-note fret label — the number trailing below each gem as
        // it travels down the highway (distinct from showFretOnNote's ghost
        // digit on the fretboard). Default true preserves existing behavior
        // (this was previously unconditional, no setting at all).
        let flyingFretLabelVisible = true;
        // Chord fret numbers at the base of the highway (per-unique-fretted-
        // position, shown so the player can read a chord shape at a glance) —
        // a THIRD, distinct fret-number element from the two above. Was fully
        // unconditional (no setting existed) until this toggle was added.
        let chordBaseFretLabelsVisible = true;
        // Dynamic fret number row (heat-coloured) — the actual row at the
        // bottom of the strings: gray labels at the standard dot positions
        // (3,5,7,9,12…) plus a gold band covering whatever fret span the
        // current chord anchor occupies (e.g. "12,13,14,15"). A FOURTH,
        // distinct fret-number element (uses pool pFretLbl, unrelated to
        // pFretColMarker/fretColumnMarkerCadence or pNoteFretLabel above).
        // Was fully unconditional (no setting existed) until this toggle.
        let dynamicFretRowVisible = true;
        // 'Show fret markers' / 'Show open-note markers' — for a plugin (e.g.
        // display_note_letter) that wants to show only note letters/labels
        // instead of the 3D gem shapes, independently for fretted vs open/
        // strum notes. Gates just the visible gem geometry (accent halo,
        // outline, lateral face fill, core body); verdict fx, scoring, and
        // the gem-position bridge stay unaffected so consumers keep working
        // with gems hidden.
        ...
        // separate chord-level "strum X" indicator drawn at a chord's frame
        // box (not gated by these — a distinct element).
        let palmMuteMarkerVisible = true;
        let fretHandMuteMarkerVisible = true;
        // 'Show chord names' — the gold chord-name label (e.g. "F#5") drawn
        // next to a chord shape. Added to match the classic 2D highway's
        // own chord-name toggle (added for display_note_letter's 2D
        ...
        // Gem-position bridge (WIP, mirrors FREECAM_BRIDGE.md's window.__h3dCamCtl
        // but read-direction): _gemProbe is a reused Vector3 for per-note NDC
        // projection; _gemBridgeFrame accumulates this frame's visible gems before
        // being published to window.__h3dGemPositions at the end of update().
        let _gemProbe = null;
        ...
        function initScene() {
            ...
            // Gem-position bridge (WIP): reused Vector3 for per-note NDC projection.
            _gemProbe = new T.Vector3();
            _gemProbe2 = new T.Vector3();
            ...
            _bgListener = (changedKey) => {
                ...
                if (changedKey === 'reactive' || changedKey === 'showFretOnNote' ||
                    changedKey === 'flyingFretLabelVisible' ||
                    changedKey === 'chordBaseFretLabelsVisible' ||
        ...
        function _bgLoadSettings() {
            ...
            if (newPalette !== activePalette || newSig !== _bgPaletteSig) {
                ...
                // Publish for external plugins (e.g. display_note_letter) that
                // want note labels to follow the same per-string colors as the
                // gems, including the user's custom/theme palette. Mirrors the
                // read direction of window.__h3dGemPositions — core writes, a
                // plugin reads.
                window.__h3dActivePalette = activePalette;
            ...
            flyingFretLabelVisible = _bgReadSetting(panelKey, 'flyingFretLabelVisible');
            chordBaseFretLabelsVisible = _bgReadSetting(panelKey, 'chordBaseFretLabelsVisible');
        ...
        function update(bundle) {
            ...
            // Gem-position bridge (WIP): start this frame's collection. Published to
            ...
            if (chords) {
                ...
                for (let ci = _chordsLoIdx; ci < chords.length; ci++) {
                    ...
                    if (chShape.size > 1 && chDt > -chordTailHoldS && chDt < AHEAD && chordOpenBoxW != null
                        ...
                        if (chordName && chordNameVisible && firstInShapeRun && !chordWireHighDensity(ch)) {
                        ...
                        if (!isRepeat && chordBaseFretLabelsVisible) {
            ...
            // Gem-position bridge (WIP) — publish this frame's collected gem NDC
            // positions for an external plugin to read. Mirrors the read direction
            // of FREECAM_BRIDGE.md's window.__h3dCamCtl: additive, opt-in, never
        ...
        function drawNote(n, now, openX, skipLabel, skipBody, linger = 0.10, openChordBoxWidth, fromChord = false, chordId, susTrailMatchArpFrame = false, arpBounds = null, prevOnsetT = -Infinity, showDropLine = false) {
            ...
            if (!effSkipBody && !arpGhostOnlyMode && !_overLinger) {
                ...
                // gemBodyVisible/openGemBodyVisible ('Show fret markers' /
                // 'Show open-note markers' settings): skips only the visible
                // gem geometry (accent halo, outline, lateral face fill, core
                // body) — verdict fx (sparks, +N pop, hit/miss marks) above and
                // the gem-position bridge push below stay unconditional, so
                ...
                if (_gemVisualsOn && n.ac && _ndState !== 'miss' && mAccentHaloNear[s]) {
                ...
                // pGhostFretLbl below.
                } // end _gemVisualsOn (outline/edges/core)

                // Gem-position bridge (WIP) — publishes this gem's NDC screen
                // position for an external plugin to read (mirrors the read
                // direction of FREECAM_BRIDGE.md's window.__h3dCamCtl: core
                // writes, a plugin reads, no plugin-specific logic lives here).
                // A second point (offset by one K-unit in world X) is projected
                // alongside the gem centre so a consumer can derive real
                // perspective scale at this gem's distance — without this, a
                // fixed-size overlay label reads oversized on far gems (which
                // have shrunk) and undersized on near ones (which have grown).
                // Deliberately placed in this unconditional per-gem block (not
                // the n.f > 0 fret-label block below) so open notes/strums
                // (n.f === 0, the wide OPEN_NOTE_WORLD_W bar) publish too —
                // they were silently missing when this lived under `n.f > 0`.
                // No alpha gate here: reaching this point already means the
                // gem mesh itself is being positioned/drawn this frame (this
                // function has no separate 0..1 fade for the gem body itself,
                // unlike the fret-label block's `alpha`, which fades a label
                // near the ends of its visible window).
                if (_gemBridgeFrame) {
                    _gemProbe.set(x, y + techniqueYNow, noteZ);
                    _gemProbe.project(cam);
                    _gemProbe2.set(x + K, y + techniqueYNow, noteZ);
                    _gemProbe2.project(cam);
                    _gemBridgeFrame.push({
                        s, f: n.f, t: n.t,
                        sx: _gemProbe.x, sy: _gemProbe.y,
                        sxK: _gemProbe2.x,
                        // Chord membership so a consumer can group/collapse chord
                        // notes (e.g. root-only or none, to cut label clutter) —
                        // chordId is undefined for some chord paths, so pair it
                        // with fromChord rather than relying on it alone.
                        ch: !!fromChord, cid: (chordId == null ? null : chordId),
                    });
                }
            ...
            if (!effSkipBody && !arpGhostOnlyMode && !_overLinger) {
                ...
                // FH/MT takes priority over PM when a note carries both flags
                // (matches the sprite-choice logic below) — the toggle check
                // has to follow the same priority, not just "either flag set".
                {
                const _wouldShowFH = !!(n.mt || n.fhm);
                const _wouldShowPM = n.pm && !_wouldShowFH;
                if ((_wouldShowFH && fretHandMuteMarkerVisible) || (_wouldShowPM && palmMuteMarkerVisible)) {
                ...
                if (n.f > 0 && !skipLabel) {
                    ...
                    // Deliberately NOT gated by flyingFretLabelVisible: confirmed live
                    // (2026-09-02) that this line is a useful height/pitch indicator on
                    // its own, independent of the fret number — hiding it alongside the
                    // number (the original assumption, that it was purely a pointer to
                    // that number) removed real information players rely on.
                    if (!fromChord) {
                    ...
                    const _showNum = flyingFretLabelVisible
            ...
            if (flyingFretLabelVisible && skipBody && fromChord && !skipLabel && n.f > 0 && dt >= 0) {
