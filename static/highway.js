...
function createHighway() {
  ...
  const hwState = {};
    ...
    // Gem/open-bar/palm-mute-marker visibility (added for display_note_letter's
    // 2D-highway support). Mirrors highway_3d's own gemBodyVisible/
    // openGemBodyVisible/palmMuteMarkerVisible split: fretted gems and the
    // open-string bar are independently hideable, each default-true (an
    // absent key reads as on). Gating happens in highway-draw.js's
    // drawNote() — the judgment glow (_paintGemGlow) and other technique
    // labels (H/P/T, tremolo, accent) stay unconditional either way, same
    // reasoning as highway_3d: a consumer standing in for the gem (e.g.
    // display_note_letter's note letters) should keep working with the gem
    // hidden, not lose hit/miss feedback too.
    hwState._dnlGemVisible = localStorage.getItem('dnlGemVisible') !== 'false';
    hwState._dnlOpenBarVisible = localStorage.getItem('dnlOpenBarVisible') !== 'false';
    // Palm-mute "PM" label (drawn on both the fretted-gem and open-bar
    // paths) AND the chord-level all-muted box+X (highway-draw.js's
    // `allMuted` block, driven by n.mt/n.fhm) are BOTH gated on this one
    // flag — Leah's call (2026-09-03): the 2D highway has no per-note
    // fret-hand-mute visual distinct from palm-mute (the "PM" text reads
    // opts.pm only; mt/fhm only ever surface at the chord-box level), so
    // routing everything through the palm-mute toggle means any song using
    // either flag gets reliably covered by the one toggle a user reaches
    // for, instead of requiring both toggles on.
    hwState._dnlPalmMuteVisible = localStorage.getItem('dnlPalmMuteVisible') !== 'false';
    // Kept for API symmetry with highway_3d/display_note_letter's setter
    // pair (window.highway.setFretHandMuteMarkerVisible is still callable
    // and won't error), but is a genuine no-op on the 2D highway today —
    // nothing in highway-draw.js reads this flag.
    hwState._dnlFretHandMuteVisible = localStorage.getItem('dnlFretHandMuteVisible') !== 'false';
    // Closest 2D equivalent to highway_3d's "Show fret number row under
    // strings" toggle: the fret-line chord preview (highway-draw.js's
    // _drawFretLineChordPreview — colored per-string dots + fret numbers
    // near the strike zone, previewing the upcoming chord). Was previously
    // always-on with no setting at all.
    hwState._dnlFretLinePreviewVisible = localStorage.getItem('dnlFretLinePreviewVisible') !== 'false';
    // Fret-number text ONLY — separate from _dnlGemVisible (the gem/bar
    // SHAPE), same split highway_3d has between its gemBodyVisible and
    // flyingFretLabelVisible. Lets a consumer keep the colored gem/string
    // cue visible while replacing just the number with something of its
    // own (display_note_letter's letters), or vice versa. Covers both the
    // fretted-gem number and the open-string bar's "0" — functionally the
    // same "which fret is this" readout, just fret 0's own shape.
    hwState._dnlFretNumberVisible = localStorage.getItem('dnlFretNumberVisible') !== 'false';
    // The chord-name label drawn above a chord shape (e.g. "F#5",
    // highway-draw.js's drawChords()) — previously unconditional except a
    // per-chord chart-authored hide flag (ch.hd), no global toggle at all.
    // Added so a consumer whose OWN chord-name display would duplicate it
    // (display_note_letter's chordMode 'name') has something to turn off.
    hwState._dnlChordNameVisible = localStorage.getItem('dnlChordNameVisible') !== 'false';
    // See drawFretNumbers()'s own comment — the persistent bottom-of-screen
    // fret ruler, the actual 2D match for highway_3d's dynamicFretRowVisible.
    hwState._dnlFretRulerVisible = localStorage.getItem('dnlFretRulerVisible') !== 'false';
  ...
  // sustained draw cost above this -> scale down
   ...
   // hard floor (lowest the user-configurable floor may be set)
    ...
    function drawFretNumbers(W, H) {
        // _dnlFretRulerVisible (added for display_note_letter's 2D-highway
        // support): the actual 2D match for highway_3d's "Show fret number
        // row under strings" (dynamicFretRowVisible) — a persistent
        // bottom-of-screen ruler, gray at standard positions + gold across
        // the current chord anchor's span. Was fully unconditional before
        // this. Distinct from _dnlFretLinePreviewVisible (the near-strike-
        // zone upcoming-chord dot preview, a different feature) — both are
        // wired to the same plugin checkbox since they're both "extra fret
        // number displays."
        if (hwState._dnlFretRulerVisible === false) return;
        const y = H * 0.97;
        const pad = 3;
        const lo = 0;
        const hi = Math.ceil(hwState.displayMaxFret);
    ...
    const api = {
        ...
        // Gem / open-bar / palm-mute marker visibility — see the hwState
        // init block above for the full rationale. Mirrors highway_3d's own
        // h3dBgSetGemBodyVisible/h3dBgSetOpenGemBodyVisible/
        // h3dBgSetPalmMuteMarkerVisible/h3dBgSetFretHandMuteMarkerVisible
        // naming so a plugin driving both renderers (display_note_letter)
        // can tell them apart at a glance. setFretHandMuteMarkerVisible is
        // wired for API symmetry but is currently a no-op — see the hwState
        // init comment on _dnlFretHandMuteVisible.
        getGemVisible() { return hwState._dnlGemVisible; },
        setGemVisible(v) {
            hwState._dnlGemVisible = !!v;
            localStorage.setItem('dnlGemVisible', String(hwState._dnlGemVisible));
        },
        getOpenBarVisible() { return hwState._dnlOpenBarVisible; },
        setOpenBarVisible(v) {
            hwState._dnlOpenBarVisible = !!v;
            localStorage.setItem('dnlOpenBarVisible', String(hwState._dnlOpenBarVisible));
        },
        getPalmMuteMarkerVisible() { return hwState._dnlPalmMuteVisible; },
        setPalmMuteMarkerVisible(v) {
            hwState._dnlPalmMuteVisible = !!v;
            localStorage.setItem('dnlPalmMuteVisible', String(hwState._dnlPalmMuteVisible));
        },
        getFretHandMuteMarkerVisible() { return hwState._dnlFretHandMuteVisible; },
        setFretHandMuteMarkerVisible(v) {
            hwState._dnlFretHandMuteVisible = !!v;
            localStorage.setItem('dnlFretHandMuteVisible', String(hwState._dnlFretHandMuteVisible));
        },
        // Closest 2D match to highway_3d's dynamicFretRowVisible — see the
        // hwState init comment on _dnlFretLinePreviewVisible.
        getFretLinePreviewVisible() { return hwState._dnlFretLinePreviewVisible; },
        setFretLinePreviewVisible(v) {
            hwState._dnlFretLinePreviewVisible = !!v;
            localStorage.setItem('dnlFretLinePreviewVisible', String(hwState._dnlFretLinePreviewVisible));
        },
        getFretNumberVisible() { return hwState._dnlFretNumberVisible; },
        setFretNumberVisible(v) {
            hwState._dnlFretNumberVisible = !!v;
            localStorage.setItem('dnlFretNumberVisible', String(hwState._dnlFretNumberVisible));
        },
        getChordNameVisible() { return hwState._dnlChordNameVisible; },
        setChordNameVisible(v) {
            hwState._dnlChordNameVisible = !!v;
            localStorage.setItem('dnlChordNameVisible', String(hwState._dnlChordNameVisible));
        },
        getFretRulerVisible() { return hwState._dnlFretRulerVisible; },
        setFretRulerVisible(v) {
            hwState._dnlFretRulerVisible = !!v;
            localStorage.setItem('dnlFretRulerVisible', String(hwState._dnlFretRulerVisible));
        },
