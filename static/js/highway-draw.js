...
export function drawNote(hwState, W, H, x, y, scale, string, fret, opts, ns) {
    ...
    // The 2D highway's only per-note mute visual is this single "PM" text
    // label, driven by opts.pm alone — there's no separate fret-hand-mute
    // marker at the per-note level. Per Leah's call (2026-09-03): gate it
    // on the palm-mute toggle ONLY (not fret-hand-mute) so a song using
    // either flag reliably gets picked up by the one toggle a user is most
    // likely to reach for, rather than requiring both to be on.
    const _dnlMuteLabelOn = hwState._dnlPalmMuteVisible !== false;
    ...
    if (fret === 0 && !isChord) {
        ...
        // _dnlOpenBarVisible / _dnlFretNumberVisible (added for
        // display_note_letter's 2D-highway support): independently gate
        // the bar shape vs. its "0" label — same body/number split as the
        // fretted-gem path above. Judgment glow stays unconditional below.
        if (hwState._dnlOpenBarVisible !== false) {
            // Shadow
        ...
        if (hwState._dnlFretNumberVisible !== false) {
            // "0" label
    ...
    // _dnlGemVisible / _dnlFretNumberVisible (added for display_note_letter's
    // 2D-highway support): independently gate the gem's own body/backing/
    // outline vs. its fret-number text — same split highway_3d has between
    // gemBodyVisible and flyingFretLabelVisible, so a consumer can keep the
    // colored gem/string cue while replacing just the number (or vice
    // versa). The judgment glow stays unconditional either way.
    const _dnlGemOn = hwState._dnlGemVisible !== false;
    const _dnlFretNumOn = hwState._dnlFretNumberVisible !== false;
    if (isHarmonic) {
...
export function drawChords(hwState, W, H) {
    ...
    for (let i = hi - 1; i >= lo; i--) {
        ...
        // allMuted is driven by n.mt/n.fhm only (fret-hand-mute — mt is just
        // the legacy field name for the same concept, kept for older chart
        // data; see highway_3d/screen.js's own fhm-vs-mt comment). Per
        // Leah's call (2026-09-03): gate on the palm-mute toggle, same as
        // the per-note "PM" text, so ANY song using either mute flag is
        // reliably covered by the one toggle rather than requiring the
        // user to also flip fret-hand-mute.
        if (allMuted && hwState._dnlPalmMuteVisible !== false) {
        ...
        // Chord name label
        if (!ch.hd && hwState._dnlChordNameVisible !== false && p.scale > 0.15 && tmpl && tmpl.name) {
        ...
        for (let j = 0; j < sorted.length; j++) {
            ...
            if (getTemplateFret(cn) === 0 && hasMultipleNotes && !_noteHasTechniqueFlags(cn)) {
                ...
                // Same body/number split as the standalone open-bar path.
                if (hwState._dnlOpenBarVisible !== false) {
                    hwState.ctx.fillStyle = dark;
                ...
                if (hwState._dnlFretNumberVisible !== false) {
                    // This bar is drawn at full `sz` height (barH above),
                    // vs. the standalone open-bar's `sz * 0.45` — so using
                    // the same `sz * 0.5` font size here read as tiny
                    // relative to this taller bar (reported 2026-09-03,
                    // confirmed live: same font formula, ~2.2x taller bar).
                    // Scale by barH using the standalone path's own
                    // font/height ratio (0.5/0.45) so both read the same
                    // proportional size, matching what "0" looks like on
                    // the standalone bar.
                    const fontSize = Math.max(8, barH * (0.5 / 0.45)) | 0;
                    hwState.ctx.fillStyle = '#fff';
                    hwState.ctx.font = `bold ${fontSize}px sans-serif`;
                    hwState.ctx.textAlign = 'center';
                    hwState.ctx.textBaseline = 'middle';
...
export function _drawFretLineChordPreview(hwState, W, H) {
    ...
    // _dnlFretLinePreviewVisible (added for display_note_letter's 2D-highway
    // support): the closest 2D match to highway_3d's "Show fret number row
    // under strings" toggle.
    if (hwState._dnlFretLinePreviewVisible === false) return;
    const strTop = H * 0.83;
    const strBot = H * 0.95;
